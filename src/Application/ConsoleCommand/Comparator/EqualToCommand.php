...
class EqualToCommand extends Command
{
    protected function configure(): void
    {
        $this->setName('comparator:eq');
        $this->setDescription(implode('; ', [
            'Check if version1 is equal to version2 according to Composer SemVer',
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $result = Comparator::equalTo($version1, $version2);
        if ($output->isVerbose()) {
            $output->writeln(sprintf(
                'version \'%s\' %s equal to version \'%s\' according to Composer SemVer',
                $version1,
                $result ? 'is' : 'is not',
                $version2
            ));
