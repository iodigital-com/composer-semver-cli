...
class LessThanCommand extends Command
{
    ...
    {
        $this->setName('comparator:lt');
        $this->setDescription(implode('; ', [
            'Check if version1 is less than version2 according to Composer SemVer',
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $result = Comparator::lessThan($version1, $version2);
        if ($output->isVerbose()) {
            $output->writeln(sprintf(
                'version \'%s\' %s less than version \'%s\' according to Composer SemVer',
                $version1,
                $result ? 'is' : 'is not',
                $version2
            ));
