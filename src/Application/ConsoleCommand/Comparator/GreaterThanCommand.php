...
class GreaterThanCommand extends Command
{
    ...
    {
        $this->setName('comparator:gt');
        $this->setDescription(implode('; ', [
            'Check if version1 is greater than version2 according to Composer SemVer',
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $result = Comparator::greaterThan($version1, $version2);
        if ($output->isVerbose()) {
            $output->writeln(sprintf(
                'version \'%s\' %s greater than version \'%s\' according to Composer SemVer',
                $version1,
                $result ? 'is' : 'is not',
                $version2
            ));
